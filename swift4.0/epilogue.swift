...
func _run_main<In: Decodable, Out: Encodable>(mainFunction: @escaping (In, @escaping (Out?, Error?) -> Void) -> Void) {
    do {
...
func _run_main<Out: Encodable>(mainFunction: @escaping ( @escaping (Out?, Error?) -> Void) -> Void) {
    let resultHandler = { (out: Out?, error: Error?) in
        if let error = error {
            _whisk_print_error(message: "Action handler callback returned an error:", error: error)
            return
